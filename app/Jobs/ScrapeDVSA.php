...
class ScrapeDVSA implements ShouldQueue
{
    ...
    // //    return $userSlots = $this->slotManager->getMatches($slots, Location::where('name', 'Skipton')->get()->first());
    ...
    //    }
        ...
        Redis::connection('default')->funnel($this->user->dl_number)->limit(1)->then(function() {
             ...
             */
                ...
                $this->slots = $this->scrapeUserLocations($this->user->locations)->map(function ($item) {
...
//            );
//            $this->slots = $this->slotManager->getMatches($slots['slots'], Location::find(1));
...
//            return $this->release(30);
    ...
    {
        // $userSlots = $this->slots->filter()->groupBy('user.id');

        // $eligibleUsers = User::whereIn('id', $userSlots->collapse()->collapse()->pluck('user.id'))->get();

        // See whether user is currently being offered a slot
        // If so, give to next best user. If no appropriate user & slot earlier
            // ? offer slot anyway - Will log out of other browser  
            // ? let it go 
            // ? Hold it with another user account until user made decision - complicaites making reservations for that user
